...
export const getSale = responseWrapper(async (req, res) => {
    ...
    if (req.query?.nazivSale) {
        salaSchema.filter = { nazivSale: req.query.nazivSale };
...
export const getSala = responseWrapper(async (req, res) => {
    const { id } = req.params;
    const result = await DBBroker.getInstance().select(new SalaSchema({ id: parseInt(id) }));
    if (result.length === 0)
...
export const addSala = responseWrapper(async (req, res) => {
    ...
    const result = await DBBroker.getInstance().insertGeneric(new SalaSchema(null, req.body));
    if (result.recordset?.length > 0) {
        const idSale = result.recordset[0].id;
        for (let sediste of listaSedista) {
            sediste.idSale = idSale;
            await DBBroker.getInstance().insertGeneric(new SedisteSchema(null, sediste));
        }
    }
    else
        throw new Error("Sala not inserted..");
    return buildApiResponse(result);
});
export const deleteSala = responseWrapper(async (req, res) => {
    await DBBroker.getInstance().beginTransaction();
    const { id } = req.params;
    const result = await DBBroker.getInstance().delete(new SalaSchema({ id: parseInt(id) }));
    return buildApiResponse(result);
});
export const patchSala = responseWrapper(async (req, res) => {
    const { id } = req.params;
    await DBBroker.getInstance().beginTransaction();
    const { listaSedista } = req.body;
    delete req.body.listaSedista;
    delete req.body.id;
    const result = await DBBroker.getInstance().patch(new SalaSchema({ id: parseInt(id) }, req.body));
    for (let sediste of listaSedista) {
        if (sediste.operation === "DELETE") {
            await DBBroker.getInstance().delete(new SedisteSchema({ idSedista: sediste.idSedista }));
        }
...
export const addSediste = responseWrapper(async (req, res) => {
    ...
    const result = await DBBroker.getInstance().insertGeneric(new SedisteSchema(null, sediste));
    return buildApiResponse(result);
});
export const removeSediste = responseWrapper(async (req, res) => {
    const result = await DBBroker.getInstance().delete(new SedisteSchema({ idSedista: parseInt(req.params.idSedista) }));
...
export const patchSediste = responseWrapper(async (req, res) => {
    ...
    const result = await DBBroker.getInstance().patch(new SedisteSchema({ idSedista: parseInt(idSedista) }, { red, kolona }));
    if (result.rowsAffected[0] === 0)
        return buildApiResponse("Ne postoji dato sediste", false, 404);
    return buildApiResponse(result);
});
