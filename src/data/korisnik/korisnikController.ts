...
export const deleteKorisnik = responseWrapper(async (req, res) => {
  ...
  const tokenId = (req as any).user.id;
  if (id !== tokenId)
    return buildApiResponse(
      "Nemate pravo da obrisete drugog korisnika",
      false,
      403
    );
  const result = await DBBroker.getInstance().delete(
    new KorisnikSchema({ id })
  );
  if (result.rowsAffected[0] === 0) throw new Error("Korisnik not found");
  return buildApiResponse(result);
});
export const addKorisnik = responseWrapper(async (req, res) => {
  ...
  const result = await DBBroker.getInstance().insertGeneric(
    new KorisnikSchema(null, korisnik)
  );
  return buildApiResponse(result);
});
export const patchKorisnik = responseWrapper(async (req, res) => {
  ...
  if (id !== tokenId)
    return buildApiResponse(
      "Nemate pravo da menjate drugog korisnika",
      false,
      403
    );
  const korisnik = req.body;
  ...
  const result = await DBBroker.getInstance().patch(
    new KorisnikSchema({ id }, korisnik)
  );
  if (result.rowsAffected[0] === 0) throw new Error("Korisnik not found");
  return buildApiResponse(result);
});
