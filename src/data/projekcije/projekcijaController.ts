...
import { Projekcija, ProjekcijaSchema } from "./projekcija.js";
...
export const getProjekcije = responseWrapper(async (req, res) => {
  ...
  if (req.query.nazivFilma) {
    ...
    const filmNameColumn = getDbColumn(
      "nazivFilma",
      projekcijaSchema.joinMeta[1].subJoin
    );
...
export const addProjekcije = responseWrapper(async (req, res) => {
  const projekcija: Projekcija = req.body;
  const result = await DBBroker.getInstance().insertGeneric(
    new ProjekcijaSchema(null, projekcija)
  );
...
export const patchProjekcija = responseWrapper(async (req, res) => {
  ...
  const result = await DBBroker.getInstance().patch(
    new ProjekcijaSchema({ id }, projekcija)
  );
  if (result.rowsAffected[0] === 0) {
    return buildApiResponse("Projekcija ne postoji", false, 404);
  }
